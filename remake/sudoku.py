...
class MainGame(Tk):
    ...
    def setupWidget(self):
        ...
        self.timer = Label(self.props_container, text="00:00", bg=self.background, fg=self.uneditable_foreground)
    ...
    def placeWidget(self):
        self.left_container.pack(padx=(20, 0), pady=20, side="left")
        self.props_container.pack(expand=True, fill='x', pady=(0, 3))
    ...
    def generateBoard(self, difficulty="hard"):
        if difficulty == "custom":
            ...
            if response:
                ...
                self.current_id = difficulty[:3].upper()+(str(response['id']).zfill(6) if difficulty != "daily" else date.today().strftime("%m%d%y"))
        ...
        self.id_label.config(text="#"+self.current_id)
        self.start_time = time.time()
        self.current_time = 0
    ...
    def startTimer(self):
        ...
        if self.is_playing and not self.is_finished:
            self.current_time += 1
